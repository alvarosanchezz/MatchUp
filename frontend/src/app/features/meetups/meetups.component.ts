...
const ESTADO_COLORS: Record<EstadoQuedada, string> = {
  ABIERTA:    'oklch(0.73 0.17 145)',
  COMPLETA:   'oklch(0.78 0.15 75)',
  FINALIZADA: 'oklch(0.55 0.02 270)',
  CANCELADA:  'oklch(0.62 0.20 25)',
};
...
export class MeetupsComponent implements OnInit, OnDestroy {
  ...
  readonly activeTab     = signal(0);   // 0 = Lista, 1 = Mapa
  ...
  applyFilters(): void { this.loadMeetups(0); }
  ...
  useMyLocation(): void {
    if (!navigator.geolocation) {
      this.snackBar.open('Geolocalización no disponible en este navegador', 'Cerrar', { duration: 3000 });
      return;
    }
    navigator.geolocation.getCurrentPosition(
      pos => {
        this.filterForm.patchValue({ lat: pos.coords.latitude, lon: pos.coords.longitude });
  ...
  goToDetail(id: number): void { this.router.navigate(['/meetups', id]); }

  sliderValue(): number { return this.filterForm.get('radioKm')?.value ?? 20; }
  ...
  private refreshMapMarkers(): void {
    ...
    for (const meetup of this.meetups()) {
      ...
      const marker = L.marker([meetup.ubicacionLatitud, meetup.ubicacionLongitud], { icon }).addTo(this.map!);
      ...
      marker.on('popupopen', () => {
        const btn = marker.getPopup()?.getElement()?.querySelector('.popup-btn') as HTMLButtonElement | null;
        if (btn) btn.onclick = () => this.router.navigate(['/meetups', meetup.id]);
      });
