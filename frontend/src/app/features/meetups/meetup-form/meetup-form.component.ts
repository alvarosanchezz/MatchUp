...
export class MeetupFormComponent implements OnDestroy {
  ...
  readonly locationSet = computed(() => this.form.get('ubicacionLatitud')?.value != null);
  ...
  onSportChange(sportId: number): void {
    ...
    if (sport) this.form.get('numJugadoresTotal')?.setValue(sport.jugadoresDefault);
  }

  displayFn(r: NominatimResult | null): string { return r?.display_name ?? ''; }
  ...
  onLocationSelect(event: MatAutocompleteSelectedEvent): void {
    ...
    this.form.patchValue({ ubicacionNombre: nombre, ubicacionLatitud: lat, ubicacionLongitud: lon });
  ...
  private initMap(): void {
    ...
    this.map.on('click', (e: L.LeafletMouseEvent) => {
      this.form.patchValue({ ubicacionLatitud: +e.latlng.lat.toFixed(6), ubicacionLongitud: +e.latlng.lng.toFixed(6) });
  ...
  private placeMarker(lat: number, lon: number): void {
    ...
    if (this.mapMarker) {
      ...
      this.mapMarker.on('dragend', () => {
        ...
        this.form.patchValue({ ubicacionLatitud: +pos.lat.toFixed(6), ubicacionLongitud: +pos.lng.toFixed(6) });
      });
    }
  }

  // ── Form helpers ──────────────────────────────────────────────────────
  get fechaFinAnterior(): boolean { return this.form.hasError('fechaFinAnterior'); }
  ...
  submit(): void {
    ...
    request$.subscribe({
      next: detail => {
        this.snackBar.open(this.isEditMode ? 'Quedada actualizada ✓' : '¡Quedada creada! 🎉', 'Cerrar', { duration: 3000 });
